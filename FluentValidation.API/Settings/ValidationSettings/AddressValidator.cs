...
public sealed class AddressValidator : AbstractValidator<Address>
{
	public AddressValidator()
	{
		...
		RuleFor(a => a.Street).Length(3, 100)
			.WithMessage(EMessage.InvalidLength.Description().FormatTo("Street", "3 to 100"));
        ...
        RuleFor(a => a.Number).Length(3, 10)
            .WithMessage(EMessage.InvalidLength.Description().FormatTo("Number", "3 to 10"));

        RuleFor(a => a.District).Length(3, 100)
            .WithMessage(EMessage.InvalidLength.Description().FormatTo("District", "3 to 100"));

        RuleFor(a => a.City).Length(3, 100)
            .WithMessage(EMessage.InvalidLength.Description().FormatTo("City", "3 to 100"));
    }
}
